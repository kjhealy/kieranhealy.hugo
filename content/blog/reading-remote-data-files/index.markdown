...
The first line inside the function uses `httr` to `GET` the file, and immediate save it to disk as a using `write_disk()`, taking care to specify that the tempfile should have an `.xlsx` extension. (Otherwise `read_xlsx()` will complain.) The second line actually reads in the file that's been downloaded. We take the opportunity to suppress chatter about the name repair that has to happen on that blank first column header in the third row, rename that location `age`, and strip the trailing line about the source of the data that I mentioned above. 
