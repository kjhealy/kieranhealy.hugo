...
When I make a PDF, I want the fonts I used to be *embedded* in the file. That way, they can be addressed directly and changed later if necessary when it comes to printing or other production. If the fonts used in your file aren't embedded in your PDF and the file is opened or printed on a system that doesn't have access to the fonts you used, they will be replaced with one of a small number of default fonts that every system or printer knows. This is bad.
...
Unfortunately, right now the only way I know to solve this (beyond just forgetting about it and using Papyrus, I mean) is to rewrite the metadata of individual OTF or TTF font files such that they can be installed as a separate font, perhaps with a different name. So you make a "new" font called "Socviz SemiCondensed" for example. You set its Regular face to be Myriad SemiCondensed. You set its bold to be Myriad Semibold SemiCondensed, its italic to be Myriad SemiCondensed Italic, and its Bold Italic to be Myriad Semibold SemiCondensed Italic. Then you save it and install it via FontBook as if it were new. This is essentially what  `systemfonts::register_variant()` lets you do in an ad hoc way in an R session if you want. But Cairo can't see those registered variants. It can only see them if you do it "for real", i.e. at the level of fonts installed in `~/Library/Fonts` or wherever. For many typefaces, this will break the terms of the license you bought it under. Applications like [TransType](https://www.fontlab.com/font-converter/transtype/) and others can do this, though they are careful to tell you, as I am telling you, that this may well be against licensing terms. You could also, possibly, only buy the specific font faces you want and install those.
